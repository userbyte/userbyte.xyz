...
function getDiscordShit() {
  fetch("https://api.lanyard.rest/v1/users/143183268571774976", {
    method: "GET",
  })
    .then((response) => response)
    .then((response) =>
      response.json().then((res) => ({ status: response.status, json: res }))
    )
    .then((response) => {
      if (response.status === 200) {
        ...
        if (resp_json["data"]["listening_to_spotify"]) {
          ...
          ct =
            ((new Date().getTime() - sp.timestamps.start) /
              (sp.timestamps.end - sp.timestamps.start)) *
            100;
          setSpotifyProgBar(ct);
          document
            .getElementById("spotify-songLink")
            .setAttribute(
              "href",
              `https://open.spotify.com/track/${sp["track_id"]}`
            );
          document
            .getElementById("spotify-albumArt")
            .setAttribute("src", sp["album_art_url"]);
          ...
          // set tooltip texts
          document
            .getElementById("spotify-artistName")
            .setAttribute("title", sp["artist"]);
          document
            .getElementById("spotify-songName")
            .setAttribute("title", sp["song"]);
          document
            .getElementById("spotify-albumName")
            .setAttribute("title", sp["album"]);
        } else {
          // console.log('userbyte is not listening to spotify');
          sp = undefined;
          if (
            document.getElementById("spotify-songLink").hasAttribute("href")
          ) {
          ...
          document
            .getElementById("spotify-albumArt")
            .setAttribute("src", "files/img/questionmark.png");
          document.getElementById("spotify-artistName").textContent =
            "---------";
...
function updateDiscordLanyardThing() {
  ...
  refreshImage(
    "lanyard-discord",
    `https://lanyard.cnrad.dev/api/143183268571774976?theme=${discordProfileCard_theme}&bg=${discordProfileCard_bgColor}&animated=${discordProfileCard_animatedPfp}&hideDiscrim=${discordProfileCard_hideDiscrim}&borderRadius=${discordProfileCard_borderRadius}&idleMessage=${discordProfileCard_idleMsg}`
  );
}
